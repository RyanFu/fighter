...
<html>
  <head>
	...
	<script type="text/javascript">
		...
		function del(myid) {
			if (confirm("确定删除这条充值记录吗？")) {
				window.location="<%=basePath%>ctrl/rechargeInfoDelete.man?rechId=" + myid;
		...
		function pageselectCallback(page_index, jq){
            ...
            for(var i=page_index*items_per_page;i<max_elem;i++)
            {
                ...
                newcontent += '<td><a href="<%=basePath%>ctrl/rechargeInfoEdit.man?rechId='+ data[i].id+'">编辑</a>&nbsp‖&nbsp<a href="javascript:void(0)" onclick="del(\'' + data[i].id + '\')">删除</a></td>';
                newcontent += '</tr>'
            }
		...
		<div id="content">
		    ...
		    <div style="float:left;margin-top:60px;margin-left:20px;width:700px;background:white;
		    	...
		    	<div style="padding-left:20px;padding-bottom:10px">
		    		...
		    		<form id="rechForm" action="<%=basePath%>ctrl/rechargeInfoSearch.man" method="post">
		    		<tr>
			    		<td>开始日期</td>
			    		<td><input name="startDate" type="text" size="6" value="${startDate}" readonly="readonly" id="startDate" onclick="new Calendar().show(this); "/></td>
			    	</tr>
			    	<tr>
			    		<td>结束日期</td>
			    		<td><input name="endDate" type="text" size="6" value="${endDate}" readonly="readonly" id="endDate" onclick="new Calendar().show(this); "/></td>
			    	</tr>
			    	<tr>
			    	    <td>结算单号</td>
			    	    <td><input type="text" name="ord_number" size="14"></td>
			    	</tr>
			    	<tr>
			    	   	<td>
		    		...
		    		<a style="padding-left:10px" href="#" id="searchRech" class="easyui-linkbutton" iconCls="icon-search">查询</a>
		    		</form>
		    	</div>
		    	<table id="adtbl">
		    		<thead>
