  ...
  <head>
	...
	<script type="text/javascript">
		$(function(){
			...
			$.ajax({
					type : 'POST',
					url : "<%=basePath%>ctrl/adCheckOtherInfo.man?adId="+adId,
				...
				for (int i = 0; i < showTypes.size(); i++) {
					...
					if ("条幅型".equals(showTypeNames[0])) {
						...
						if ("显示文字＋图片".equals(showTypeNames[1])) {
							...
							out.println("$('#imgPath3').attr('src','"+tomcatServerIp+"/files/ad/"+ast.getAd_info_url()+"');");
							out.println("$('#imgOnly').hide();");
						} else if ("仅显示图片".equals(showTypeNames[1])) {
							out.println("$('#imgPath2').attr('src','"+tomcatServerIp+"/files/ad/"+ast.getAd_info_url()+"');");
						...
						out.println("$('#uploadInsertPath').attr('src','"+tomcatServerIp+"/files/fullad/"+ast.getAd_info_url()+"');");
						...
						if ("显示文字＋图片".equals(showTypeNames[1])) {
							...
							out.println("$('#imgPath32').attr('src','"+tomcatServerIp+"/files/ad/"+ast.getAd_info_url()+"');");
							out.println("$('#imgOnly2').hide();");
						} else if ("仅显示图片".equals(showTypeNames[1])) {
							out.println("$('#imgPath22').attr('src','"+tomcatServerIp+"/files/ad/"+ast.getAd_info_url()+"');");
  	...
  	<div id="head">
		...
		<div id="content" style="margin: 0 auto;width:972px;white-space:nowrap;">
		    ...
		    <div class="Border" style="font-family: Arial, Helvetica, sans-serif;font-size: 12px">
		    	...
		    	<h2 style="margin-top:20px;margin-left:60px">广告发布——设置展现方式</h2>
		    	<div >
		    	...
		    	<div class="borderDark">
		    		<div><input id="adpass1" name="adpass" type="checkbox" value="未通过">不通过</div>
		    		<div><input id="adpass2" name="adpass" type="checkbox" value="运行中">通过</div>
		    	</div>
		    	<div style="margin-left:20px;padding-top:20px;padding-left:20px;width:640px;">
	    			<button type="button" id="success">完成</button>
	    		</div>
			    </form>
			    </div>
		    </div>
